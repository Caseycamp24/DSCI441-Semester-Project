...
def _read_csv_lean(filepath: str, source_name: str) -> pd.DataFrame:
    """Read only the columns we need from a single CSV."""
    # Peek at headers first so we request only existing columns
    header = pd.read_csv(filepath, nrows=0)
    use = [c for c in header.columns
           if c in _KEEP_COLS or not c.startswith("Unnamed")]
...
def load_data(path: str):
    ...
    full_df = pd.concat(
        (_read_csv_lean(os.path.join(path, f), f) for f in csv_files),
        ignore_index=True,
    )
