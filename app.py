...
_CLOUD_SAMPLE_FRAC = 0.5  # use 50 % of rows on the cloud — still millions of shots
...
def _read_csv_lean(filepath: str, source_name: str) -> pd.DataFrame:
    """Read only the columns we need from a single CSV with compact dtypes."""
    header = pd.read_csv(filepath, nrows=0)
    use    = [c for c in header.columns
              if c in _KEEP_COLS and not c.startswith("Unnamed")]
    dtype  = {c: v for c, v in _DTYPE_MAP.items() if c in use}
    df     = pd.read_csv(filepath, usecols=use, dtype=dtype, low_memory=False)
