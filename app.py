...
def _kaggle_download() -> str:
    """Download dataset via kagglehub and return path to the nba/ subfolder."""
    import kagglehub  # imported lazily — not needed for local runs
    base = kagglehub.dataset_download(_KAGGLE_DATASET)
    # kagglehub returns the version root; CSVs live in the 'nba' subfolder
    candidate = os.path.join(base, "nba")
    if os.path.isdir(candidate):
        return candidate
    # Some versions put CSVs directly in the root
    return base
